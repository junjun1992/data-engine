# -*- coding: utf-8 -*-
"""
Created on Mon Mar 22 11:00:56 2021
...
for param in parameters_list:
    try:
        #model = ARIMA(df_month.Count,order=(param[0], param[1], param[2])).fit()
        # SARIMAX 包含季节趋势因素的ARIMA模型
        model = sm.tsa.statespace.SARIMAX(df_month.Count,
...
for i in range(future_month):
    ...
    # print(next_month_days)怎么都是31天呢？？？？
...
df_month2.Count.plot(label='实际数量')
df_month2.forecast.plot(color='r', ls='--', label='预测数量')
plt.legend()
plt.title('乘客数量（月）')
plt.xlabel('时间')
plt.ylabel('数量')
